...
class Tree {
    ...
    function get_treeview_menu($myid,$effected_id='example', $str="<span class='file'>\$name</span>", $str2="<span class='folder'>\$name</span>", $showlevel = 0,  $ul_class="" ,$li_class="" , $style='filetree ', $currentlevel = 1, $recursion=FALSE, $dropdown='hasChild') {
    	...
    	foreach ($child as $id => $a) {
			...
			if($currentlevel==1){
				$this->str .= $recursion ? "<ul><li  $floder_status id= 'menu-item-$id'>" : "<li  $floder_status   id= 'menu-item-$id'>";
    			$recursion = FALSE;
    			//判断是否为终极栏目
    			if ($this->get_child($a['id'])) {
    				eval("\$nstr = \"$str2\";");
    				$this->str .= $nstr;
					$this->str .= "<div cl='$currentlevel' sl='$showlevel' class='$ul_class'><div>";
                    //eval("\$nstr = \"$str\";");
    			    //$this->str .= $nstr;
    		        //$this->str .='</p>';
    				if (($showlevel > 0 && $showlevel > $currentlevel)) {
    			...
    			$this->str .=$recursion ? '<div></div></li></ul>' : '</li>';
			}
			
			else if($currentlevel==2){
				$this->str .= $recursion ? "<div cl='$currentlevel' sl='$showlevel'><p>" : "<li  $floder_status   id= 'menu-item-$id'>";
    	          eval("\$nstr = \"$str\";");
    			  $this->str .= $nstr;
				  $this->str .= "</p>";
				//判断是否为终极栏目
    		   if ($this->get_child($a['id'])) {
    			...
    			$this->str .=$recursion ? '</div>' : '</li>';
				
			}
		
			else{
	    		$this->str .= $recursion ? "<ul cl='$currentlevel' sl='$showlevel'><li  $floder_status id= 'menu-item-$id'>" : "<li  $floder_status   id= 'menu-item-$id'>";
	    		$recursion = FALSE;
	    		//判断是否为终极栏目
	    		if ($this->get_child($a['id'])) {
	    			eval("\$nstr = \"$str2\";");
